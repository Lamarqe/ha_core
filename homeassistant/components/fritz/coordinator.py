...
from .mesh import FritzMeshTopology
from .models import ConnectionInfo, Device, FritzDevice, HostAttributes, HostInfo
...
class FritzBoxTools(DataUpdateCoordinator[UpdateCoordinatorDataType]):
    ...
    def __init__(
        ...
        self._mesh: FritzMeshTopology | None = None
    ...
    def mesh_nodes(self) -> dict[str, str]:
        ...
        return self._mesh.mesh_nodes if self._mesh else {}
    ...
    def node_uplink_rates(self) -> dict[str, tuple[int | None, int | None]]:
        ...
        return self._mesh.node_uplink_rates if self._mesh else {}

    @property
    def slave_parent_nodes(self) -> dict[str, str]:
        """Return slave parent node names mapping sensor MAC to parent node name."""
        return self._mesh.slave_parent_nodes if self._mesh else {}
        ...
        if hosts_attributes:
            for attributes in hosts_attributes:
                ...
                hosts[attributes["MACAddress"]] = Device(
                    name=attributes.get("X_AVM-DE_FriendlyName")
                    or attributes["HostName"],
    ...
    async def async_trigger_cleanup(self) -> None:
        ...
        valid_connections.update(
            {(CONNECTION_NETWORK_MAC, mac) for mac in self.mesh_nodes.values()}
        )
        valid_identifiers = {(DOMAIN, switch_key) for switch_key in self.switch_nodes}
